 ...
 * <p>
 * If a node is removed that was required _in its parent node,
...
public class ModifierVisitor<A> implements GenericVisitor<Visitable, A> {
    ...
    private <N:Node> NodeList<N> modifyList(NodeList<N> list, A arg) {
        return (NodeList<N>) list.accept(this, arg);
    }

    private <N:Node> NodeList<N> modifyList(Optional<NodeList<N>> list, A arg) {
