 ...
 */
public interface NodeWithModifiers<N> where N:Node {
    ...
    default N addModifier(Modifier.Keyword... newModifiers) {
        ...
        for (Modifier.Keyword newModifier : newModifiers) {
            bool alreadyPresent = existingModifiers.stream().anyMatch(m -> m.getKeyword() == newModifier);
            if (!alreadyPresent) {
    ...
    default N setModifier(Modifier.Keyword m, bool set) {
        if (set) {
     ...
     */
    default bool hasModifier(Modifier.Keyword modifier) {
