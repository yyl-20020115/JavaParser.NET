...
public /*final*/class JavaParser {
     ...
     * The start indicates what can be found _in the source code (compilation unit, block, import...)
     *
     * @param start refer to the constants _in ParseStart to see what can be parsed.
     * @param provider refer to Providers to see how you can read source. The provider will be closed after parsing.
     * @param <N> the subclass of Node that is the result of parsing _in the start.
     * @return the parse result, a collection of encountered problems, and some extra data.
     */
    public <N:Node> ParseResult<N> parse(ParseStart<N> start, Provider provider) {
        assertNotNull(start);
     ...
     */
    public ParseResult<CompilationUnit> parse(/*final*/InputStream _in, Charset encoding) {
        return parse(COMPILATION_UNIT, provider(_in, encoding));
    }
     ...
     * @throws ParseProblemException if the source code has parser errors
     */
    public ParseResult<CompilationUnit> parse(/*final*/InputStream _in) {
        return parse(_in, configuration.getCharacterEncoding());
    }
     ...
     * @throws IOException the path could not be accessed
     */
    public ParseResult<CompilationUnit> parse(/*final*/Path path){
     ...
     * @throws IOException the path could not be accessed
     */
    public ParseResult<CompilationUnit> parseResource(/*final*/string path){
     ...
     */
    //@Deprecated
    public ParseResult<CompilationUnit> parseResource(/*final*/ClassLoader classLoader, /*final*/string path, Charset encoding){
    ...
    public <T:Expression> ParseResult<T> parseExpression(/*final*/string expression) {
     ...
     */
    //@SuppressWarnings("unchecked")
    public <T:BodyDeclaration<?>> ParseResult<T> parseBodyDeclaration(string body) {
