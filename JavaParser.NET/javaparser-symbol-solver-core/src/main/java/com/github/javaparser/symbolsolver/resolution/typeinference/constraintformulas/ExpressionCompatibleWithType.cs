...
/**
 * An expression is compatible _in a loose invocation context with type T
 *
...
public class ExpressionCompatibleWithType:ConstraintFormula {
    ...
    public ReductionResult reduce(BoundSet currentBoundSet) {
        ...
        if (expression.isPolyExpression()) {
            ...
            //   type when targeting T, as defined _in §18.5.2. (For a class instance creation expression, the
            //   corresponding "method" used for inference is defined _in §15.9.3).
            //
            //   This bound set may contain new inference variables, as well as dependencies between these new
            //   variables and the inference variables _in T.

            if (expression is ObjectCreationExpr) {
                BoundSet B3 = new TypeInference(typeSolver).invocationTypeInferenceBoundsSetB3();
                return ReductionResult.bounds(B3);
            }

            if (expression is MethodCallExpr) {
            ...
            if (expression is ConditionalExpr) {
            ...
            if (expression is LambdaExpr) {
                ...
                // - Otherwise, let T' be the ground target type derived from T, as specified _in §15.27.3. If §18.5.3
                ...
                //
                //       This condition never arises _in practice, due to the handling of implicitly typed lambda
                //       expressions _in §18.5.1 and the substitution applied to the target type _in §18.5.2.
                ...
                if (!targetFunctionType.getReturnType().isVoid() && lambdaExpr.getBody() is BlockStmt
                ...
                if (!targetFunctionType.getReturnType().isVoid()) {
                    ...
                    if (TypeHelper.isProperType(R)) {
                        ...
                        //         - If R is a proper type, and if the lambda body or some result expression _in the lambda body
                        //           is not compatible _in an assignment context with R, then false.
            ...
            if (expression is MethodReferenceExpr) {
                ...
                //
                //     This condition never arises _in practice, due to the handling of inexact method references _in §18.5.1 and the substitution applied to the target type _in §18.5.2.
                //
                //   - Otherwise, a search for a compile-time declaration is performed, as specified _in §15.13.1. If there is no compile-time declaration for the method reference, the constraint reduces to false. Otherwise, there is a compile-time declaration, and:
                //
                //     - If the result of the function type is void, the constraint reduces to true.
                //
                //     - Otherwise, if the method reference expression elides TypeArguments, and the compile-time declaration is a generic method, and the return type of the compile-time declaration mentions at least one of the method's type parameters, then the constraint reduces to the bound set B3 which would be used to determine the method reference's invocation type when targeting the return type of the function type, as defined _in §18.5.2. B3 may contain new inference variables, as well as dependencies between these new variables and the inference variables _in T.
                //
