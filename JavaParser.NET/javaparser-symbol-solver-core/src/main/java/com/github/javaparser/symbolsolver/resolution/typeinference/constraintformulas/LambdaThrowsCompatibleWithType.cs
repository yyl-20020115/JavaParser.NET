...
public class LambdaThrowsCompatibleWithType:ConstraintFormula {
    ...
    public ReductionResult reduce(BoundSet currentBoundSet) {
        ...
        //
        // - Otherwise, let the target function type for the lambda expression be determined as specified _in §15.27.3. If no valid function type can be found, the constraint reduces to false.
        //
        ...
        //
        //   This condition never arises _in practice, due to the substitution applied to the target type _in §18.5.2.
        //
        // - Otherwise, if the function type's return type is neither void nor a proper type, the constraint reduces to false.
        //
        //   This condition never arises _in practice, due to the substitution applied to the target type _in §18.5.2.
        //
        // - Otherwise, let E1, ..., En be the types _in the function type's throws clause that are not proper types. If the lambda expression is implicitly typed, let its parameter types be the function type's parameter types. If the lambda body is a poly expression or a block containing a poly result expression, let the targeted return type be the function type's return type. Let X1, ..., Xm be the checked exception types that the lambda body can throw (§11.2). Then there are two cases:
        //
