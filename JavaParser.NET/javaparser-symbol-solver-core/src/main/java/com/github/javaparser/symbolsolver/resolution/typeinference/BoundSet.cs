...
public class BoundSet {
    ...
    private bool areSameTypeInference(ResolvedType a, ResolvedType b) {
    ...
    private bool hasInstantiationFor(InferenceVariable v) {
        for (Bound b : bounds) {
        ...
        for (Set<InferenceVariable> alphas: allSetsWithProperty(uninstantiatedPortionOfV, dependencies)) {
            ...
            if (!hasSomeCaptureForAlphas) {
                ...
                for (InferenceVariable alphaI : alphas) {
                    ...
                    bool throwsBound = bounds.stream().anyMatch(b -> b.isThrowsBoundOn(alphaI));
    ...
    private bool properUpperBoundsAreAtMostExceptionThrowableAndObject(InferenceVariable inferenceVariable) {
        throw new UnsupportedOperationException();
    }
